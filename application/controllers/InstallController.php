...
class InstallController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        try {
            ...
            if($t==count($this->tables)){
                 $this->view->error='It seems that the script is already installed.If you want to reinstall,then empty the database'; 
                  return 0;
            }elseif($t>0&&$t<count($this->tables)){
                $this->view->error='It seems that the script is not  installed right.Please reinstall'; 
                return 0;
            }            
            
            
        if ($this->getRequest()->isPost()) {
             if ($iForm->isValid($_POST)) {
                ...
                try {    
                    ...
                    $db->query(" CREATE TABLE IF NOT EXISTS `content_nodes` (
                    ...
                    $db->query("INSERT INTO `menus` (`id`, `name`, `access_level`, `position`) VALUES
                    ...
                    $db->query("INSERT INTO `menu_items` (`id`, `menu_id`, `label`, `link_type`, `page_cat_id`, `page_id`, `link`, `position`) VALUES
								(1, 2, 'Home', 'link', 0, 0, '/', 2),
                    ...
                    $db->query("CREATE TABLE IF NOT EXISTS `options` (
                      ...
                      $title=$iForm->getValue('title');
                      $keywords=$iForm->getValue('meta_keywords');
                      $description=$iForm->getValue('meta_description');
                      $cemail=$iForm->getValue('email');
                    
                    $db->query("INSERT INTO `options` (`id`, `title`, `meta_description`, `meta_keywords`, `cEmail`) VALUES
					(1, '$title', '$description', '$keywords', '$cemail') ");;
                    
                     $db->query("CREATE TABLE IF NOT EXISTS `orderdetails` (
  							`id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
                     ...
                     $db->query("CREATE TABLE IF NOT EXISTS `products` (
                       ...
                       $db->query("CREATE TABLE IF NOT EXISTS `product_cat` (
  							`id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
 							 `name` varchar(100) NOT NULL,
                      ...
                      $username=$iForm->getValue('admin_user');
                      $password=md5($iForm->getValue('admin_pass'));
                       
                       $db->query(" INSERT INTO `users` (`id`, `role`, `username`, `password`, `first_name`, `last_name`, `address`, `state`, `zip`, `city`, `country`) VALUES
            ...
            $iForm->setAction('/install/');              
            $this->view->form=$iForm;
